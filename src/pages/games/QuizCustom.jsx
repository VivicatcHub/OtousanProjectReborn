...
export default function QuizCustom() {
  const { t: translate } = useTranslation();
  const navigate = useNavigate();
  const { languages } = useData();
  const { known, learn } = useSettings();
  const [values, set] = useCustomSettings("quiz", DEFAULTS);
  const { categories, canPlay, ready } = useGameCategories("quiz", {
  ...
  const play = () =>
    navigate(
      `/games/quiz/play?${new URLSearchParams({
        dir: values.dir,
        image: values.image,
  ...
  return (
    <div className="space-y-6">
      <BackLink to="/games/quiz" label={translate("common.back")} />
      <div>
        <h1 className="text-3xl font-black">
          {translate("quizSetup.title")} — {translate("common.custom")}
        </h1>
